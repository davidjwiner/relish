...
The package under `convex/components/browserbase` owns the reusable client, Agent tool adapter, browser and Stagehand APIs, validation, provider transport, and deployable lifecycle component. Convex components cannot run Node actions, so the host keeps a one-line `browserActions.execute` registration for the SDK runtime and `lib/browser.ts` supplies Relish's generated references. Consumers initialize the public `Browserbase` export with `new Browserbase(components.browserbase, internal.browserActions.execute)`. Stagehand is pinned to **3.7.3** for its reconnect and keep-alive disconnect behavior; upgrading to v4 requires revisiting that lifecycle. Node dependencies are configured in `convex.json`.

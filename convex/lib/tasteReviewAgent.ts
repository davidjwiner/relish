...
export type TasteReviewOutput = {
  overview: string;
  overviewEvidenceIds: string[];
  drawnTo: TasteReviewClaim[];
  avoids: TasteReviewClaim[];
  nuances: TasteReviewClaim[];
  evidenceLevel: 'limited' | 'developing';
};

type TasteReviewClaim = {
  text: string;
  evidenceIds: string[];
};

export function tasteReviewOutputFor(
  evidenceIds: [string, ...string[]],
): z.ZodType<TasteReviewOutput> {
  ...
  return z.object({
    ...
    overviewEvidenceIds: z.array(evidenceId).min(1).max(5),
